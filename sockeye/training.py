...
class EarlyStoppingTrainer:
    ...
    def _forward_backward(self, batch: data_io.Batch, is_update_batch: bool = True):
        ...
        for factor_num in range(1, batch.source.size(2)):
            ...
            print(f'Input batch: device={batch.source.device} factor_num={factor_num} data_min={data_min} '
                  f'data_max={data_max}')
        batch = batch.load(device=self.device)
        for factor_num in range(1, batch.source.size(2)):
            data_min = torch.min(batch.source[:, :, factor_num])
            data_max = torch.max(batch.source[:, :, factor_num])
            print(f'Loaded batch: device={batch.source.device} factor_num={factor_num} data_min={data_min} '
                  f'data_max={data_max}')
